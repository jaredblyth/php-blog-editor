...
if (isset($_GET['id']) && is_numeric($_GET['id']) ) { // Display the entry in a form:
	...
	if ($r = mysql_query($query, $dbc)) { // Run the query.
		...
		<p align="center"><h2><font color="red">Are you sure you want to delete this entry?</font></h2></p>
		<p align="center"><h3>' . $row['title'] . '</h3><br />
		<p align="center"><input type="hidden" name="id" value="' . $_GET['id'] . '" />
		<p align="center"><input type="submit" name="submit" value="Delete this Entry!" /></p>
		</form>';
	...
	// Define the query:
	$query = "DELETE FROM blog WHERE entry_id={$_POST['id']} LIMIT 1";
	$r = mysql_query($query, $dbc); // Execute the query.
	
	// Report on the result:
	if (mysql_affected_rows($dbc) == 1) {
		print '<p align="center"><font color="red">The blog entry has been deleted.</font></p>';
		header( 'Location: view-entries.php?result=3' );
	} else {
